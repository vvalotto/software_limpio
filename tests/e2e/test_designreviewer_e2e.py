...
class TestDesignReviewerPipelineCompleto:
    ...
    def test_17_analyzers_ejecutados(self, proyecto_limpio):
        """Deben ejecutarse los 17 analyzers del DesignReviewer."""
        runner = CliRunner()
        result = runner.invoke(main, [str(proyecto_limpio), "--format", "json"])

        data = json.loads(result.output)
        assert data["summary"]["analyzers_executed"] == 17
...
class TestDogfooding:
    ...
    def test_dogfooding_json_campos_completos(self):
        ...
        assert summary["analyzers_executed"] == 17
